...
Private Sub cmdAdd_Click()
        With m_ucmStorage
            If .StorageReady Then
                If .AddUpdateItem(txtItemPath.Text, vbNull, txtMemo.Text, enumStorageType.File, Val(txtItemData.Text), txtCategory.Text, chkAutoUpdate.Value = vbChecked) = enumAddUpdateItemResult.OK Then
                    OpenStorage
...
Private Sub cmdDelete_Click()
        m_ucmStorage.DeleteItem Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))
        picContent.Cls
        txtContent.Text = ""
        OpenStorage
End Sub
        ...
        With colItems
            ...
            For m_lngItems = 1 To .Count
                With .Item(m_lngItems)
                    ...
                    lvwRecords.SubItemSet m_lngItems - 1, 3, m_ucmStorage.GetTotalSize(m_lngItems), 0
                    lvwRecords.SubItemSet m_lngItems - 1, 4, .strCRC, 0
                    lvwRecords.SubItemSet m_lngItems - 1, 5, m_ucmStorage.GetStorageTypeName(.udeType), 0
                    lvwRecords.SubItemSet m_lngItems - 1, 6, .strCategory, 0
                    lvwRecords.SubItemSet m_lngItems - 1, 7, .strKey, 0
                    lvwRecords.SubItemSet m_lngItems - 1, 8, .strMemo, 0
                End With
            Next
        End With
...
Private Sub Form_Load()
        ...
        With lvwRecords
            ...
            Call .ColumnAdd(4, "CRC", 80, [caLeft])
            Call .ColumnAdd(5, "Type", 40, [caLeft])
            Call .ColumnAdd(6, "Category", 80, [caRight])
            Call .ColumnAdd(7, "Path", 200, [caLeft])
            Call .ColumnAdd(8, "Memo", 100, [caLeft])
        ...
        With m_ucmStorage
            ...
            If o_blnResult Then
                ...
                If blnDirect Then
                    ...
                    txtItemData.Text = ""
                Else
                    o_strType = .Items.Item(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strType
                    txtMemo.Text = .Items.Item(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strMemo
                    txtItemData.Text = .Items.Item(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).lngItemData
                End If
                ...
                Select Case LCase(o_strType)
                    Case "jpeg", "jpg", "bmp", "gif", "wmf", "ico", "rle"
                        ...
                        Else
                            Set picContent.Picture = .GetItemPicture(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0)))
                        End If
                    ...
                    Case "txt", "pif", "bat", "htm", "html", "js", "dhtml", "asp", "jsp", "vbp", "frm", "cls", "bas", "vb", "cs", "c", "h", "cpp", "hpp"
                        ...
                        Else
                            txtContent.Text = .GetItemText(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0)))
                        End If
...
Private Sub SaveItem()
        If m_ucmStorage.Items.Count > 0 And lvwRecords.SelectedItem > -1 Then
            With New cFileDlg
                .Filters.AddFilter m_ucmStorage.Items(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strType & "ÎÄ¼þ", "*." & m_ucmStorage.Items(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strType
                .FilterIndex = 2
                .flags = ofnPathMustExist Or ofnOverwritePrompt
                .FileName = m_ucmFile.GetFileName(m_ucmStorage.Items(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strKey)
                If .ShowSave(Me.hWnd) Then
                    If InStrRev(.FileName, "." & m_ucmStorage.Items(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strType) = 0 Then
                        .FileName = .FileName & "." & m_ucmStorage.Items(Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0))).strType
                    End If
                    
                    m_ucmStorage.SaveItemToFile Val(lvwRecords.SubItemText(lvwRecords.SelectedItem, 0)), .FileName
...
Private Sub txtFilter_KeyPress(KeyAscii As Integer)
        If KeyAscii = vbKeyReturn Then
            ...
            If chkFilters(5).Value = vbChecked Then o_udeFilter = o_udeFilter Or Memo
            If chkFilters(6).Value = vbChecked Then o_udeFilter = o_udeFilter Or Category
